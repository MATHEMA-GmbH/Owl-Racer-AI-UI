...
{
    internal class ModelButton : Component
    {
        ...
        public ScriptMappingOptions script { get; set; }

        private int maxLetters = 17;
        ...
        public ModelButton(ScriptMappingOptions script, Texture2D texture, SpriteFont font, float scale, string filepath)
        {
            _texture = texture;
            ...
            if(script.CarName != null ) 
            {
                interimText = script.CarName;

            }
            else if(script.File != null)
            {
                interimText = script.File;
            }
            else
            {
                interimText = script.Key;
            }
            int textLen = Math.Min(maxLetters, interimText.Length);
            Text = interimText.Substring(interimText.Length - textLen, textLen);
